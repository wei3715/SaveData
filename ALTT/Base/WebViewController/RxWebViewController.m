...
//    RLog(@"push with request %@",request);
    NSURLRequest* lastRequest = (NSURLRequest*)[[self.snapShotsArray lastObject] objectForKey:@"request"];
    
    //如果url是很奇怪的就不push
    if ([request.URL.absoluteString isEqualToString:@"about:blank"]) {
//        RLog(@"about blank!! return");
        return;
    }
    //如果url一样就不进行push
...
//    RLog(@"now array count %d",self.snapShotsArray.count);
}
...
//    RLog(@"prev center x%f",prevSnapshotViewCenter.x);
...
//    RLog(@"pan x %f,pan y %f",translation.x,translation.y);
...
//    RLog(@"navigation type %d",navigationType);
